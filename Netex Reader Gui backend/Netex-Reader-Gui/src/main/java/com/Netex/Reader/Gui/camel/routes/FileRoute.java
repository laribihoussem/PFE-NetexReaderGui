...
public class FileRoute extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from("file:files/output")
                ...
                .choice()
                .when(SNCF)
                    .to("sftp://tester@192.168.1.5:2222//RebexTinySftpServer//data//SNCF?password=password")
                    .to("bean:GlobalService?method=send")
                    .to("kafka:SNCF?brokers=localhost:9092")
                .when(STIF)
                    .to("sftp://tester@192.168.1.5:2222//RebexTinySftpServer//data//STIF?password=password")
                    .to("bean:GlobalService?method=send")
                    .to("kafka:STIF?brokers=localhost:9092")
                .when(RATP)
                    .to("sftp://tester@192.168.1.5:2222//RebexTinySftpServer//data//RATP?password=password")
                    .to("bean:GlobalService?method=send")
